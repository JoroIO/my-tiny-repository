<?php
	...
	if ($mood=='Happy'){
		$mood_query = "Genre LIKE '%action%' OR Genre LIKE '%comedy%' OR Genre LIKE '%fantasy%'";
	}
	if ($mood=='Sad'){
		$mood_query = "Genre LIKE '%drama%' OR Genre LIKE '%romantic%'";
	}
	if ($mood=='Depressed'){
		$mood_query = "Genre LIKE '%comedy%' OR Genre LIKE '%drama%' OR Genre LIKE '%horror%'";
	}
	if ($mood=='Loved'){
		$mood_query = "Genre LIKE '%comedy%' OR Genre LIKE '%romantic%' OR Genre LIKE '%fantasy%'";
	}
	...
	//Filter by mood only
	if (empty($film_title) && empty($genre) && empty($film_year) && !empty($mood)){
		$query = "SELECT ID,Title,Year,Description,Image FROM films WHERE $mood_query" ;
	}
	...
	//Filter by title and mood
	if (!empty($film_title) && empty($genre) && empty($film_year) && !empty($mood)){
		$query="SELECT ID,Title,Year,Description,Image FROM films WHERE $title_query AND ($mood_query)" ;
	}
	
	//Filter by genre and mood
	if (empty($film_title) && !empty($genre) && empty($film_year) && !empty($mood)){
		$query="SELECT ID,Title,Year,Description,Image FROM films WHERE (ID IN (SELECT DISTINCT(Film_ID) FROM genres WHERE Genre IN ($genre_list) GROUP BY Film_ID HAVING COUNT(*)>=$length)) AND ($mood_query)" ;
	}
	
	//Filter by year and mood
	if (empty($film_title) && empty($genre) && !empty($film_year) && !empty($mood)){
		$query="SELECT ID,Title,Year,Description,Image FROM films WHERE Year=$film_year AND ($mood_query)" ;
	}
	...
	//Filter by title, genre and mood
	if (!empty($film_title) && !empty($genre) && empty($film_year) && !empty($mood)){
		$query="SELECT ID,Title,Year,Description,Image FROM films WHERE $title_query AND (ID IN (SELECT DISTINCT(Film_ID) FROM genres WHERE Genre IN ($genre_list) GROUP BY Film_ID HAVING COUNT(*)>=$length)) AND ($mood_query)" ;
	}
	
	//Filter by year, genre and mood
	if (empty($film_title) && !empty($genre) && !empty($film_year) && !empty($mood)){
		$query="SELECT ID,Title,Year,Description,Image FROM films WHERE(ID IN (SELECT DISTINCT(Film_ID) FROM genres WHERE Genre IN ($genre_list) GROUP BY Film_ID HAVING COUNT(*)>=$length)) AND Year=$film_year AND($mood_query)" ;
	}
	
	//Filter by title, year and mood
	if (!empty($film_title) && empty($genre) && !empty($film_year) && !empty($mood)){
		$query="SELECT ID,Title,Year,Description,Image FROM films WHERE $title_query AND Year=$film_year AND($mood_query)" ;
	}
	
	//Filter by title, genre, year and mood
	if (!empty($film_title) && !empty($genre) && !empty($film_year) && !empty($mood)){
		$query="SELECT ID,Title,Year,Description,Image FROM films WHERE $title_query AND (ID IN (SELECT DISTINCT(Film_ID) FROM genres WHERE Genre IN ($genre_list) GROUP BY Film_ID HAVING COUNT(*)>=$length)) AND Year=$film_year AND ($mood_query)" ;
	}
...
<body>
    ...
    <div class="container">
		...
		<form action="registered_user.php" method="get">
		<div class="row">
			<div class="box">
				...
				<div class="row">
						...
						<div class="col-lg-2">
							<select name="mood">
								...
								<option value="Depressed" <?php if($mood == 'Depressed') echo "selected"; ?> >Depressed</option>
								<option value="Loved" <?php if($mood == 'Loved') echo "selected"; ?> >Loved</option>
							</select>
						</div>	
				</div>
				...
				<div class="row">
					<div class="col-lg-12">
						<hr>
						<h2 class="intro-text text-center">choose your
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="col-lg-1 col-lg-offset-3">
							<img class="skin_color" id="pink" src="img/skin_color_pink.png"/>
						</div>
						<div class="col-lg-1">
							<img class="skin_color" id="violet" src="img/skin_color_violet.png"/>
						</div>
						<div class="col-lg-1">
							<img class="skin_color" id="blue" src="img/skin_color_blue.png"/>
						</div>
						<div class="col-lg-1">
							<img class="skin_color" id="green" src="img/skin_color_green.png"/>
						</div>	
						<div class="col-lg-1">	
							<img class="skin_color" id="yellow" src="img/skin_color_yellow.png"/>
						</div>
						<div class="col-lg-1">
							<img class="skin_color" id="orange" src="img/skin_color_orange.png"/>
						</div>	
						<div class="col-lg-1">
							<img class="skin_color" id="reset" src="img/skin_color_reset.png"/>
						</div>
					</div>
				</div>	
			</div>
	...
	<script>
		$('.skin_color').click(function() {
			var id = $(this).attr('id');
			if ( id == "pink" ){
				$('body').css({'background-image':'none','background-color':'#f0489b'});
			} else if ( id == "violet" ) {
				$('body').css({'background-image':'none','background-color':'#44108a'});
			} else if ( id == "blue" ) {
				$('body').css({'background-image':'none','background-color':'#112a7d'});
			} else if ( id == "green" ) {
				$('body').css({'background-image':'none','background-color':'#60a912'});
			} else if ( id == "yellow" ) {
				$('body').css({'background-image':'none','background-color':'#f0e648'});
			} else if ( id == "orange" ) {	
				$('body').css({'background-image':'none','background-color':'#f89f12'});
			} else if ( id == "reset" ) {
